...
while len(lista_palavras) > 0:
    ...
    palavra = lista_palavras[indice].strip()
...
pontos_jogador_atual = qtd_palavras - len(lista_palavras)
print(f"Você fez {pontos_jogador_atual} pontos!\n")

nome_jogador_atual = input("Digite seu nome: ")

ranking = []
ranking.append((nome_jogador_atual, pontos_jogador_atual))

with caminho_saida.open("a+", encoding="utf-8") as saida:
    saida.seek(0)
    for linha in saida:
        partes = linha.split(":")
        nome = partes[0].strip()
        pontos = int(partes[1].strip())
